...
from helpers import DataFrames, DpsWord, ResourcePaths, get_resource_paths_test, parse_data_frames
from html_components import render_header_tmpl, render_feedback_tmpl_test, render_word_meaning_test


def generate_html_and_json_test(generate_roots: bool = True):
    ...
    for row in range(df_length):
        ...
        if w.meaning != "":
            html_string += f"""<a class="button_test" href="javascript:void(0);" onclick="button_click(this)" data-target="grammar_test_{w.pali_}">grammar</a>"""

        if w.eg1 != "" and w.eg2 != "" and w.eg3 != "":
            html_string += f"""<a class="button_test" href="javascript:void(0);" onclick="button_click(this)" data-target="example_test_{w.pali_}">examples</a>"""
        ...
        if w.eg1 != "" and w.eg2 == "" and w.eg3 == "":
            html_string += f"""<a class="button_test" href="javascript:void(0);" onclick="button_click(this)" data-target="example_test_{w.pali_}">example</a>"""
        ...
        if w.pos in conjugations:
            html_string += f"""<a class="button_test" href="javascript:void(0);" onclick="button_click(this)" data-target="conjugation_test_{w.pali_}">conjugation</a>"""

        if w.pos in declensions:
            html_string += f"""<a class="button_test" href="javascript:void(0);" onclick="button_click(this)" data-target="declension_test_{w.pali_}">declension</a>"""

        html_string += f"""<a class="button_test" href="javascript:void(0);" onclick="button_click(this)" data-target="feedback_test_{w.pali_}">feedback</a>"""
        html_string += f"""</div>"""

        # grammar

        html_string += f"""<div id="grammar_test_{w.pali_}" class="content_test hidden">"""
        html_string += f"""<table class = "table1_test">"""
        if w.pos != "":
        ...
        html_string += f"""<div id="example_test_{w.pali_}" class="content_test hidden">"""

        html_string += f"""<p>"""

        if w.eg1 != "":

            html_string += f"""{w.eg1}<p class="sutta_test">{w.source1} {w.sutta1}</p>"""

            if w.chapter1 != "":
                html_string += f"""<p class="sutta_test">{w.chapter1}"""
        ...
        if w.eg2 != "":
            ...
            html_string += f"""<p>{w.eg2}<p class="sutta_test">{w.source2} {w.sutta2}</p>"""

            if w.chapter2 != "":
                html_string += f"""<p class="sutta_test">{w.chapter2}"""
        ...
        if w.eg3 != "":
            ...
            html_string += f"""<p> {w.eg3}<p class="sutta_test"> {w.source3} {w.sutta3}</p>"""

            if w.chapter3 != "":
                html_string += f"""<p class="sutta_test"> {w.chapter3}"""
        ...
        html_string += f"""<p class="sutta_test">{w.sbs_index}</p>"""
        ...
        if w.pos not in indeclinables:
            ...
            if w.pos in declensions:
                ...
                html_string += f"""<div id="declension_test_{w.pali_}" class="content_test hidden">"""

            if w.pos in conjugations:

                html_string += f"""<div id="conjugation_test_{w.pali_}" class="content_test hidden">"""
        ...
        html_string += render_feedback_tmpl_test(w)
...
# write text versions
    ...
    p = rsc['output_share_dir'].joinpath("test_full.txt")
    with open(p, "w", encoding ="utf-8") as f:
        f.write(text_data_full)

    p = rsc['output_share_dir'].joinpath("test_concise.txt")
    with open(p, "w", encoding ="utf-8") as f:
