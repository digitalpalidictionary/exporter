...
def render_word_meaning(w: DpdWord) -> RenderResult:
    ...
    if w.meaning == "":
        ...
        if w.base != "" and w.pos != "fut":
            ...
            construction_truncated = re.sub(" > .[^ ]+", "", construction_oneline)
            ...
            if re.match("^na ", w.construction):
                ...
                construction_truncated = re.sub(r"na > a|na > an|na > ana", "", construction_truncated)
            else:
                construction_na = ""
        ...
        if w.base != "" and w.pos == "fut":
            base = re.sub(" > .+ \\(.+\\)$", "", w.base)   # remove > base and end brackets
            base = re.sub("√.[^ ]* \\+ ", "", base)
            family_prefix = re.sub(" ", " + ", w.family)
            construction_truncated = re.sub(
                " > .[^ ]+", "", w.construction)  # remove phonetic changes
            construction_truncated = re.sub(
                r"(.+)( \+ .*$)", "\\2", construction_truncated)

            if re.match("^na ", w.construction):
                construction_na = re.sub(
                    "^(na )(.+)$", "\\1+ ", w.construction)
                construction_truncated = re.sub(
                    r"na > a|na > an|na > ana", "", construction_truncated)
            else:
                construction_na = ""

            construction_reconstructed = f"{construction_na}{family_prefix} + {base}{construction_truncated}"
            html_string += f""" [{construction_reconstructed}]"""
            text_concise += f""" [{construction_reconstructed}]"""
